...
public class MazeGenerator : MonoBehaviour
{
    ...
    public bool YellowKeyActive;

    public bool YellowDoorActive;
    ...
    public void CreateLayout()
    {
        ...
        for (int x = 1; x <= mazeColumns; x++)
        {
            for (int y = 1; y <= mazeRows; y++)
            {
                ...
                int RandomKeyInt = Random.Range(1, 11);
                if (RandomKeyInt == 2)
                {
                    if (!YellowKeyActive)
                    {
                        GameObject YellowKey = Instantiate(yellowKeyPrefab);
                        YellowKey.transform.position = new Vector3(spawnPos.x, spawnPos.y, -1);
                        YellowKeyActive = true;
                    }
                }
    ...
    public void MakeExit()
    {
        ...
        //EDIT - spawn door oject at exit
        if (YellowKeyActive)
        {
            GameObject ExitDoor = Instantiate(yellowDoorPrefab);
            GameObject ExitPoint = Instantiate(exitPrefab);
            if (newCell.gridPos.x == 0)
            {
                //left wall removed
                Debug.Log("placed door to the left");
                ExitDoor.transform.position = new Vector3(newCell.cellObject.transform.position.x -1, newCell.cellObject.transform.position.y, -1);
                ExitPoint.transform.position = new Vector3(newCell.cellObject.transform.position.x -1, newCell.cellObject.transform.position.y, -1);
            }
            else if (newCell.gridPos.x== mazeColumns)
            {
                //
                Debug.Log("placed door to the right");
                ExitDoor.transform.position = new Vector3(newCell.cellObject.transform.position.x + 1, newCell.cellObject.transform.position.y, -1);
                ExitPoint.transform.position = new Vector3(newCell.cellObject.transform.position.x + 1, newCell.cellObject.transform.position.y, -1);
            }
            ...
            {
                //
                Debug.Log("placed door up");
                ExitDoor.transform.position = new Vector3(newCell.cellObject.transform.position.x, newCell.cellObject.transform.position.y + 1, -1);
                ExitPoint.transform.position = new Vector3(newCell.cellObject.transform.position.x, newCell.cellObject.transform.position.y + 1, -1);
            }
            else
            {
                Debug.Log("place door down");
                ExitDoor.transform.position = new Vector3(newCell.cellObject.transform.position.x, newCell.cellObject.transform.position.y - 1, -1);
                ExitPoint.transform.position = new Vector3(newCell.cellObject.transform.position.x, newCell.cellObject.transform.position.y - 1, -1);
            }

            YellowDoorActive = true;
        }
    ...
    public void DeleteMaze()
    {
        if (mazeParent != null)
        {
            ...
            GameObject[] leftoverDoors = GameObject.FindGameObjectsWithTag("Door");

            GameObject[] leftoverKeys = GameObject.FindGameObjectsWithTag("key");

            GameObject [] leftoverExits = GameObject.FindGameObjectsWithTag("EndPoint");

            foreach (GameObject door in leftoverDoors) {
                Destroy(door);
            }

            foreach (GameObject key in leftoverKeys)
            {
                Destroy(key);
            }
            foreach (GameObject exit in leftoverExits)
            {
                Destroy(exit);
            }

            YellowKeyActive = false;
            YellowDoorActive = false;
        }
    }
